...
VtbCover_top_std::VtbCover_top_std() = default;
VtbCover_top_std::~VtbCover_top_std() = default;

void VtbCover_top_std::ctor(VtbCover_top__Syms* symsp, const char* namep) {
    vlSymsp = symsp;
    vlNamep = strdup(Verilated::catName(vlSymsp->name(), namep));
    // Reset structure values
}

void VtbCover_top_std___configure_coverage(VtbCover_top_std* vlSelf, bool first);

void VtbCover_top_std::__Vconfigure(bool first) {
    (void)first;  // Prevent unused variable warning
    VtbCover_top_std___configure_coverage(this, first);
}

void VtbCover_top_std::dtor() {
    VL_DO_DANGLING(std::free(const_cast<char*>(vlNamep)), vlNamep);
}

// Coverage
void VtbCover_top_std::__vlCoverInsert(uint32_t* countp, bool enable, bool localCounter, const char* filenamep, int lineno, int column,
...
// Toggle Coverage
void VtbCover_top_std::__vlCoverToggleInsert(int begin, int end, bool ranged, uint32_t* countp, bool enable, bool localCounter, const char* filenamep, int lineno, int column,
